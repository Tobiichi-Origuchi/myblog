...
document.addEventListener("DOMContentLoaded", function () {
  ...
  let zola21Blocks = Array.from(
    document.querySelectorAll("pre[class^='language-']"),
  );
  let blocks = [...zola22Blocks, ...zola21Blocks].filter(
    (block) => !block.closest("div.crt"),
  );
  ...
  blocks.forEach((block) => {
    if (navigator.clipboard) {
      ...
      let lang =
        (codeElement &&
          (codeElement.getAttribute("data-name") ||
      ...
      title.innerHTML = langMap[lang] || lang;
